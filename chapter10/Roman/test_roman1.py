...
class KnownValues(unittest.TestCase):
    ...
    def test_to_roman_known_values(self):
        """to_roman_should give known result withh known input"""
...
class FromRomanBadInput(unittest.TestCase):
    def test_too_many_repeated_numerals(self):
        ...
        for s in ('MMM','DD', 'CCCC','LL','VV','IIII'):
            self.assertRaises(roman1.InvalidRomanNumeralError, roman1.from_roman, s)

    def test_repeated_pairs(self):
        """from_roman should fail with repeated pairs of numerals"""
        for s in ('CMCM', 'CDCD', 'XCXC', 'XLXL', 'IXIX', 'IVIV'):
            self.aseertRaises(roman1.InvalidRomanNumeralError, roman1.from_roman, s)
